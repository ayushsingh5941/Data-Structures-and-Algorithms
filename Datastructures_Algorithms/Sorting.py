...
def counting_sort(array):
    ''' Counting sort '''
    key = list(set(array)) # To store key positions
    k = len(key)

    l1 = [0] * k # Initializing array to store counts
    for i in range(k):
        l1[i] = []

    for i in range(len(array)):
        l1[key.index(array[i])].append(array[i]) # appending each value to respective position
    
    output = []
    for i in range(k):
        output.extend(l1[i]) # Combining all sorted values
    return output
